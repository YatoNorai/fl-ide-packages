...
termux_step_post_get_source() {
	...
	# Fix clock skew in Docker/WSL2: file timestamps can appear 1-2s in the
	# future, causing make to detect Makefile.PL as "newer than Makefile",
	# triggering an infinite regeneration loop that exits with 'false'.
	# Touching all files normalizes them to the current clock.
	echo "Normalizing source timestamps to fix Docker/WSL2 clock skew..."
...
termux_step_make() {
	# Perl's build generates Makefile.PL files on-the-fly via miniperl_top.
	# Under Docker/WSL2, these new files land with a timestamp ~1-2s in the
	# future (clock skew between WSL2 host and container). make sees
	# Makefile.PL as newer than Makefile, tries to regenerate, runs 'false',
	# and fails. The fix: retry make up to 10 times, touching all files
	# before each attempt to collapse future timestamps to "now".
	...
	while true; do
		attempt=$((attempt + 1))
		echo "[perl-make] Attempt $attempt / $MAX_RETRIES"

		# Touch everything so no file appears to be in the future
		find . -exec touch {} + 2>/dev/null || true

		if make -j "$TERMUX_PKG_MAKE_PROCESSES" ${TERMUX_PKG_EXTRA_MAKE_ARGS}; then
			echo "[perl-make] Build succeeded on attempt $attempt"
			return 0
		fi

		# Check if the failure is the known clock-skew Makefile loop
		# (make exits with code 2 and the pattern is always the same).
		# If we've exhausted retries, propagate the failure.
		if [ "$attempt" -ge "$MAX_RETRIES" ]; then
			echo "[perl-make] Giving up after $MAX_RETRIES attempts." >&2
			return 1
		fi

		echo "[perl-make] make failed — likely clock skew. Touching files and retrying..."
		sleep 2
	done
}
